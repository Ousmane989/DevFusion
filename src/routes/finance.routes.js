...
async function buildFinance(userId, currency) {
  const orders = (await db.prepare("SELECT total_mru, created_at FROM orders WHERE user_id = ? AND status = 'livree'").all(userId))
    .map((o) => ({ total: o.total_mru, d: new Date(String(o.created_at).replace(' ', 'T') + 'Z') }));
  const expenses = await db.prepare('SELECT id, label, category, amount_mru, spent_on FROM expenses WHERE user_id = ? ORDER BY spent_on DESC, id DESC').all(userId);
...
router.get('/', requireAuth, async (req, res) => {
  res.json(await buildFinance(req.user.id, req.user.currency || 'MRU'));
});
...
router.post('/expense', requireAuth, async (req, res) => {
  ...
  await db.prepare('INSERT INTO expenses (user_id, label, category, amount_mru, spent_on) VALUES (?, ?, ?, ?, ?)')
    .run(req.user.id, label, category, amount, date);
  res.status(201).json(await buildFinance(req.user.id, req.user.currency || 'MRU'));
});
...
router.get('/export.csv', requireAuth, async (req, res) => {
  ...
  (await db.prepare("SELECT ref, total_mru, created_at FROM orders WHERE user_id = ? AND status = 'livree' ORDER BY created_at").all(req.user.id))
    .forEach((o) => rows.push(['vente', String(o.created_at).slice(0, 10), o.ref, 'Commande livrée', o.total_mru, '']));
  (await db.prepare('SELECT label, category, amount_mru, spent_on FROM expenses WHERE user_id = ? ORDER BY spent_on').all(req.user.id))
