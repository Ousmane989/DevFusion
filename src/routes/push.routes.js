...
router.get('/key', requireAuth, async (_req, res) => {
  ...
  const count = await push.countSubscriptions(_req.account.id);
...
router.post('/subscribe', requireAuth, async (req, res) => {
  try {
    await push.saveSubscription(req.account.id, req.body && req.body.subscription);
    res.json({ ok: true });
  } catch (e) {
...
router.post('/unsubscribe', requireAuth, async (req, res) => {
  ...
  if (endpoint) await push.removeSubscription(req.account.id, endpoint);
  res.json({ ok: true });
});

// POST /api/push/test — envoie une notification de démonstration
router.post('/test', requireAuth, async (req, res) => {
  await push.sendToUser(req.account.id, {
